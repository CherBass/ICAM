...
from model_utils import deconv3x3_2d, conv3x3_2d
...
class NetEc(nn.Module):
    def __init__(self, opt):
        ...
        tch = opt.tch
        enc_c += [LeakyReLUConv2d(input_dim, tch, kernel_size=3, stride=1, padding=0)]
        num_layers = 2
...
class NetEa(nn.Module):
    def __init__(self, opt):
        ...
        conv_layers += [nn.Conv2d(input_dim, ndf, kernel_size=4, stride=2, padding=0, bias=True)]
...
class NetGen(nn.Module):
    def __init__(self, opt):
        ...
        dec2 = [_make_layer(self, BasicBlock_up_2d, tch//2, 1, down=False, in_planes=tch, use_upsample=False)]
        tch = tch//2

        dec3 = [_make_layer(self, BasicBlock_up_2d, tch//2, 1, down=False, in_planes=tch, use_upsample=False)]
        tch = tch//2
...
class NetDisContent(nn.Module):
    def __init__(self, opt):
        ...
        c_dim = opt.num_domains
        model = []
        model += [LeakyReLUConv2d(tch, tch, kernel_size=3, stride=2, padding=1, norm='Instance')]
        model += [LeakyReLUConv2d(tch, tch, kernel_size=3, stride=2, padding=1, norm='Instance')]
        model += [LeakyReLUConv2d(tch, tch, kernel_size=3, stride=2, padding=1, norm='Instance')]
        model += [LeakyReLUConv2d(tch, tch, kernel_size=4, stride=1, padding=0)]
...
class BasicBlock_up_2d(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, use_upsample=False):
        super(BasicBlock_up_2d, self).__init__()
        ...
        self.shortcut = deconv3x3_2d(inplanes, planes, use_upsample=use_upsample)
...
def _make_layer(self, block, planes, blocks, stride=1, down=True, in_planes=None, use_upsample=False):
    ...
    if down:
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = conv3x3_2d(self.inplanes, planes * block.expansion, stride)
...
class GaussianLayer(nn.Module):
    def __init__(self, input_dim):
        ...
        self.seq = nn.Sequential(
            ...
            nn.Conv2d(input_dim, input_dim, 21, stride=1, padding=0, bias=None, groups=input_dim)
        )
